...
- [ ] 9.1 Dockerfile backend (Daphne) и frontend (multi-stage: build SPA → nginx со статикой и прокси `/api`, `/ws`)
- [ ] 9.2 `docker-compose.yml` в `justfans-backend`: db, redis, backend (миграции на старте), frontend (build из git URL `justfans-frontend`); `.env.example` с порогами
- [ ] 9.3 README backend: запуск, порты, учётки из сидера, эмуляция фана (кнопка + curl), env-пороги, ссылка на деплой и на репо спецификации; README frontend: dev-запуск
